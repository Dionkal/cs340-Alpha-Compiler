...
void readFile(std::string filename)
{
	...
	unsigned const_string_size, const_num_size, user_func_size, lib_func_size, instr_size;
	...
	/*Get the size of the program vars*/
	std::getline(input, line);
	globalSize = (unsigned)std::stoi(line, &sz);

	/*Get all the array sizes*/
	std::getline(input, line);
	const_string_size = (unsigned)std::stoi(line, &sz);
	std::getline(input, line);
	const_num_size = (unsigned)std::stoi(line, &sz);
	std::getline(input, line);
	lib_func_size = (unsigned)std::stoi(line, &sz);
	std::getline(input, line);
	user_func_size = (unsigned)std::stoi(line, &sz);
	std::getline(input, line);
	instr_size = (unsigned)std::stoi(line, &sz);
